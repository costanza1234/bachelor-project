...
At the same time, Large Language Models (LLMs) like Chat-GPT\footnote{\url{https://openai.com/chatgpt}} and Gemini\footnote{\url{https://gemini.google.com}} have become increasingly popular among the general population. Although these models were not originally designed to replace Search Engines, many people now use them to look up information they would otherwise search for online \cite{Mendel2024Laypeople’s, Divekar2024Choosing, Fernández-Pichel2024Evaluating}. The single response offered by an LLM is sometimes seen as more convenient than browsing a list of search results \cite{Stadler2024Cognitive, Caramancion2024Large, Worledge2024The}. Because of this, LLMs and Search Engines are often perceived as interchangeable, even though they work very differently \cite{Caramancion2024Large, Lindemann2023Sealed, Worledge2024The, Xiong2024When}. Such tools are also quickly evolving: new models include web browsing capabilities, allowing them to access real-time information. These systems combine LLMs with live web access\footnote{\url{https://openai.com/index/introducing-chatgpt-search/}} to provide up-to-date answers, further closing the gap between conversational assistants and traditional Search Engines \cite{Xiong2024When, Shi2024Know, Venkit2024Search, Fernández-Pichel2024Evaluating}.
...
This project builds on Savoia's 2025 thesis~\cite{Savoia2025}, which proposed a game to observe children's search behavior using both a traditional Search Engine and a chatbot-like LLM interface. Because the original prototype was a proof of concept, it did not implement a data collection feature. This thesis rebuilds that tool with the goal of creating a functioning and reusable prototype that can be used in real research scenarios and contribute to the development of child-friendly search tools.
...
Inspired by the approach of \citet{Landoni2021c}, we extended the analysis by identifying the following types of data as relevant.
...
\begin{itemize}
    \item There is no one-size-fits-all SERP (Search Engine Results Page)  interface suitable for children \cite{Landoni2021}.
...
\begin{itemize}
    \item \textbf{Google Search API} Performs a custom Google search and retrieves the top 10 results for a given query. When users select the \textit{Google} option in \texttt{ChoicePage}, this API returns Search Engine results, each including metadata such as title, snippet, rank, whether it was clicked, click order, and time spent on the result.
...
All of this information can be exported through the \texttt{exportData()} function and analyzed to study user behavior, preference trends, and differences between information retrieval strategies. An example of exported data is shown in Appendix~\ref{app:exported-data}.
...
The tool was presented to three primary school teachers with experience working with children and technology. In particular, they run computer-related lectures in classroom settings in Ticino. Here, children typically begin using Google in third grade under close guidance. At this stage, experts report children's struggle with navigating the SERP, switching between tabs, and identifying which results to click on. They provided valuable feedback on the design and usability of the tool, which should be discussed during future iterations of development. Please note that the feedback should be considered as input to be discussed and further analyzed, rather than as definitive requirements to be implemented.
...
\begin{enumerate}
    ...
    \item One teacher raised concern about the lack of scaffolding during the research phase of the game, as younger students may require structured support to conduct meaningful searches. It was suggested to provide curated websites alongside the guiding questions, ensuring that students are able to find suitable information independently without being overwhelmed.
    \item The choice page was appreciated by the experts and considered a good idea for encouraging decision-making during the activity.
\end{enumerate}
...
This project focused on the design and development of a tool to support researchers collecting user data to study how children interact with different information retrieval systems. The tool, designed in collaboration with researchers and teachers and built as an interactive web-based game, allows children to choose between a traditional Search Engine and a chatbot-style LLM to answer emotionally framed questions. All interactions are logged in a structured way, allowing the collection of data to support research.

The work originated from the limitations identified in Savoia's thesis \cite{Savoia2025}, particularly the need for data collection features. Although her work laid the foundation for a possible approach to studying search behavior, it did not provide a usable or extensible tool for research purposes. This project rebuilt the system with the goals mentioned above in mind.
