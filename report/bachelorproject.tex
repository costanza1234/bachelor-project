...
As children become more active users of the web both at home and in school, understanding how they search for information has become increasingly important in both Information Retrieval (IR) and Human-Computer Interaction (HCI). However, most search tools are designed for adults and do not consider the cognitive and emotional needs of younger users.
Researchers highlight the need of tools that allow them to collect structured interaction data, in order to gather more quantitative data on children's behaviors when searching for information online.
This thesis presents an interactive web-based game designed as a tool to study how children search using either a traditional search engine or a Large Language Model (LLM). The game is built to be engaging for children and practical for researchers. It collects detailed data such as queries, tool preference, session length, and user responses while allowing easy export of anonymized session logs.
The project rebuilds and extends an earlier prototype by making the system modular, reusable, and ready for real-world testing. Initial feedback from education experts confirms its potential as a research tool and highlights areas for further UI improvement. This tool provides a foundation for future studies on children's information-seeking behavior.

\vspace{0.5em}

\textbf{Keywords}: Information Retrieval; Human-Computer Interaction; User Interfare; Search Engine; LLM; Children;
}
